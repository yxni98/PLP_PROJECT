...
def randCent(dataSet, k):
    ...
    for j in range(n): # 创建随机簇质心，并且在每一维的边界内
        ...
        centroids[:,j] = minJ + rangeJ * np.random.rand(k)    # 随机生成
    return centroids
...
def kMeans(dataSet, k, init_cent, distMeas=distEclud, createCent=randCent):
    m = np.shape(dataSet)[0]    # 行数
    clusterAssment = np.zeros((m, 2))    # 创建一个与 dataSet 行数一样，但是有两列的矩阵，用来保存簇分配结果
